...
export default function CenterTranscript({ utterances, contradiction, onDismissContradiction, currentStreaming }: CenterTranscriptProps) {
  ...
  const liveHint = currentStreaming ? `Live token ${currentStreaming.wordIndex + 1}` : 'Live stream idle';

  return (
    <section className="min-w-0 h-full rounded-2xl border overflow-hidden flex flex-col" style={{ background: 'hsl(var(--bg-card) / 0.86)', borderColor: 'hsl(var(--border-subtle))', boxShadow: '0 8px 18px hsl(var(--court-navy) / 0.04)' }}>
      <div className="flex flex-wrap items-center gap-2 md:gap-3 px-4 py-3 md:px-5 flex-shrink-0" style={{ borderBottom: '1px solid hsl(var(--border-subtle))' }}>
        <span className="label-gold flex-shrink-0">Live Transcript</span>
        <span className="text-[10px] uppercase tracking-[0.9px] px-2 py-0.5 rounded-full" style={{ background: 'hsl(var(--court-green) / 0.1)', color: 'hsl(var(--court-green))' }}>{liveHint}</span>
        <div className="flex items-center gap-2 w-full md:w-auto md:flex-1 md:max-w-[280px]">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-court pb-1 max-w-full">
          {FILTERS.map(f => (
            <button
              key={f.value}
              onClick={() => setFilter(f.value)}
              className="px-2.5 py-1 rounded-full whitespace-nowrap text-[10px] font-semibold uppercase tracking-[0.7px] transition-all"
              style={{
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto scrollbar-court p-4 md:p-5 space-y-3">
        {filtered.length === 0 && (
...
function UtteranceBlock({ utterance: u, getConfidenceStyle }: { utterance: Utterance; getConfidenceStyle: (c: number) => React.CSSProperties }) {
  return (
    <article className={`rounded-xl p-4 animate-utterance-enter ${ROLE_BORDER_CLASS[u.role]}`} style={{ background: 'hsl(var(--bg-light) / 0.8)', boxShadow: '0 1px 0 hsl(var(--border-subtle) / 0.45)' }}>
